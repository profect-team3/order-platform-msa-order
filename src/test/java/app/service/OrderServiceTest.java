...
class OrderServiceTest {
	...
	void createOrder_Success() {
		...
		when(internalStoreClient.isStoreExists(storeId)).thenReturn(true);
		when(internalStoreClient.getMenuInfoList(menuIds)).thenReturn(menuInfoResponseList);
	...
	void createOrder_StoreNotFound() {
		...
		when(cartService.getCartFromCache(userId)).thenReturn(cartItems);
		when(internalStoreClient.isStoreExists(storeId)).thenReturn(false);
		// When & Then
